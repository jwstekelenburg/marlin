...
Compose `api` / `worker` / `spider` all `depends_on: migrate` completed successfully. Fresh volume → migrate runs automatically. Existing data → run migrate before new app versions.
